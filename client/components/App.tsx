...
function App() {
  ...
  return (
    <>
      <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-gray-100 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            <Link 
              to="/developer" 
              className={`text-sm font-bold transition-colors no-underline ${
            ...
            <div className="relative">
              <span className="text-xl group-hover:rotate-12 transition-transform block" aria-hidden="true">
                🛒
              </span>
              {totalItems > 0 && (
                <span className="absolute -top-2 -right-2 bg-price text-white text-xs font-black w-5 h-5 flex items-center justify-center rounded-full border-2 border-kiwi-dark animate-in zoom-in">
                  {totalItems}
                </span>
              )}
            </div>
          ...
          <div className="relative z-10 max-w-7xl mx-auto">
            ...
            <div className="mt-10 flex items-center gap-6 opacity-80">
              <div className="flex flex-col">
                ...
                <span className="text-xs uppercase font-bold tracking-widest text-kiwi-light">
                  Products
                </span>
              </div>
      ...
      <main className={`${isHomePage ? 'max-w-7xl mx-auto py-10 px-4 md:px-8' : ''}`}>
        <Outlet />
      </main>
